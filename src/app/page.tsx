        ...
        <>
          ...
          {rest.length > 0 && (
            <section className="mt-24">
              ...
              <div className="mt-7 grid border-l border-t border-rule/15 sm:grid-cols-2 lg:grid-cols-3">
...
function Masthead({ postCount }: { postCount: number }) {
  return (
    <section className="mb-14 grid min-h-[46vh] items-end gap-10 border-b border-rule/15 pb-12 lg:grid-cols-[1fr_auto]">
      <div className="max-w-5xl">
        <div className="eyebrow"><span className="text-accent">{siteConfig.tagline}</span> · {siteConfig.edition}</div>
        <h1 className="mt-6 font-display text-[clamp(3.4rem,10vw,8.5rem)] font-semibold leading-[0.92] tracking-[-0.035em]">
      ...
      <div className="hidden w-44 border-l border-rule/15 pl-6 font-mono text-[10px] uppercase leading-loose tracking-[0.18em] text-muted lg:block">
...
function SectionRule({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-5">
      ...
      <div className="h-px flex-1 bg-rule/15" />
...
function LeadStory({ post }: { post: ListedPost }) {
  ...
  return (
    <article className="card-lift grid overflow-hidden border border-rule/15 hover:border-rule/30 lg:grid-cols-[1.18fr_0.82fr]">
      <Link href={`/blog/${slug}`} className="min-h-[420px] overflow-hidden border-b border-rule/15 lg:border-b-0 lg:border-r">
...
function PostCard({ post, index }: { post: ListedPost; index: number }) {
  const { slug, frontmatter, readingTimeMin } = post;
  return (
    <article className="card-lift group flex flex-col border-b border-r border-rule/15 p-5 sm:p-6">
      ...
      <div className="mt-7 border-t border-rule/10 pt-4 font-mono text-[10px] uppercase tracking-[0.18em] text-muted">
...
function EmptyState() {
  return <div className="border border-dashed border-rule/20 py-24 text-center font-display text-3xl">No signals in the archive.</div>;
}
